...
## Use `setwd()` if needed
setwd("X:/mds/DSC520 - Statistics for Data Science/dsc520")

## Load the file `data/tidynomicon/person.csv` to `person_df1` using `read.csv`
## Examine the structure of `person_df1` using `str()`
person_df1 <- read.csv("X:/mds/DSC520 - Statistics for Data Science/dsc520/data/tidynomicon/person.csv", stringsAsFactors = TRUE)
str(person_df1)
...
person_df2 <- read.csv("X:/mds/DSC520 - Statistics for Data Science/dsc520/data/tidynomicon/person.csv",stringsAsFactors = FALSE)
str(person_df2)

## Read the file `data/scores.csv` to `scores_df`
## Display summary statistics using the `summary()` function
scores_df <- read.csv("X:/mds/DSC520 - Statistics for Data Science/dsc520/data/scores.csv")
columns <- sapply(scores_df, is.character) # assigning all values that are a character (evaluating to TRUE as a character) to "columns"
scores_df[columns] <- lapply(scores_df[columns], factor) # converting the characters into factors
summary(scores_df)
...
excel_sheets("X:/mds/DSC520 - Statistics for Data Science/dsc520/data/G04ResultsDetail2004-11-02.xls")
...
voter_turnout_df1 <- read_excel("X:/mds/DSC520 - Statistics for Data Science/dsc520/data/G04ResultsDetail2004-11-02.xls",sheet="Voter Turnout", skip = 1)
...
voter_turnout_df2 <- read_excel("X:/mds/DSC520 - Statistics for Data Science/dsc520/data/G04ResultsDetail2004-11-02.xls",sheet="Voter Turnout", skip = 2, col_names = c("ward_precint", "ballots_cast", "registered_voters", "voter_turnout"))
...
db <- dbConnect(RSQLite::SQLite(),"X:/mds/DSC520 - Statistics for Data Science/dsc520/data/tidynomicon/example.db")
