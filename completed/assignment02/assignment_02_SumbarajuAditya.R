...
path <- "data/tidynomicon/person.csv"
person_df1 <- read.csv(file = path, stringsAsFactors = TRUE)
str(person_df1)
#person_df1
...
#person_df3 <- read.csv(file.path("$path","person.csv"), stringsAsFactors = FALSE)
person_df2 <- read.csv(file = path, stringsAsFactors = FALSE)
#str(person_df2)
#str(person_df3)
str(person_df2)
## Read the file `data/scores.csv` to `scores_df`
## Display summary statistics using the `summary()` function
path1 <- "data/scores.csv"
scores_df <- read.csv(file = path1)
cols <- sapply(scores_df, is.character)
scores_df[cols] <- lapply(scores_df[cols], factor)
summary(scores_df)
#scores_df$Section <-as.character(scores_df$Section)
#scores_df1 <-  read.csv("data/scores.csv")
#summary(scores_df$Section)
#summary(scores_df1)
...
path2="data/G04ResultsDetail2004-11-02.xls"
...
table_names
#site_df <- dbGetQuery(db,"SELECT * FROM SITE")
#head(site_df)
#Visited_df <- dbGetQuery(db,"SELECT * FROM VISITED")
#head(Visited_df,n=8)
