...
## Use `setwd()` if needed
setwd("/Users/carla/Documents/GitHub/dsc520")

## Load the file `data/tidynomicon/person.csv` to `person_df1`
##  using `read.csv`#Examine the structure of `person_df1` using `str()`
person_df1 <- read.csv("C:\\Users\\carla\\Documents\\GitHub\\dsc520\\data\\tidynomicon\\person.csv", header=T, sep=",")
str(person_df1)
...
person_df2 <- read.csv("C:\\Users\\carla\\Documents\\GitHub\\dsc520\\data\\tidynomicon\\person.csv", header=T, sep=",", stringsAsFactors = FALSE)
str(person_df1)

## Read the file `data/scores.csv` to `scores_df`
## Display summary statistics using the `summary()` function
scores_df <- read.csv("data\\scores.csv", header=T, sep=",")
summary(scores_df)
...
voter_turnout_df2 <- read_excel("data/G04ResultsDetail2004-11-02.xls", sheet="Voter Turnout",col_names = FALSE, skip = 2)
