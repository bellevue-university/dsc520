...
## sex vs. earn
ggplot(heights_df, aes(x=sex, y=earn)) + geom_point()+ geom_boxplot(notch = FALSE,varwidth = TRUE,outlier.colour = "red", outlier.shape = 1)
## race vs. earn
ggplot(heights_df, aes(x=race, y=earn)) + geom_point()+ geom_boxplot(notch = FALSE,varwidth = TRUE,outlier.colour = "red", outlier.shape = 1)

# https://ggplot2.tidyverse.org/reference/geom_bar.html
## Using `geom_bar()` plot a bar chart of the number of records for each `sex`
ggplot(heights_df, aes(sex)) + geom_bar(aes(fill = race))

## Using `geom_bar()` plot a bar chart of the number of records for each race
ggplot(heights_df, aes(race)) + geom_bar(aes(fill = sex))

## Create a horizontal bar chart by adding `coord_flip()` to the previous plot
ggplot(heights_df, aes(race))+ geom_bar(aes(fill = sex)) + coord_flip()
...
head(covid_df$date,n=5)
head(format(covid_df$date, format="%B %d %Y"), n=5)
...
#ggplot(data=ny_df, aes(x=date, y=cases, group=1)) + geom_line(aes(colour = cases)) + scale_colour_gradient(high="red")
