...
for(i in 1:12){
  model <- kmeans(df, i)
  df$label <- as.factor(model$cluster)
  centers <- as.data.frame(model$centers)
  df$distance <- sqrt((df$x - model$centers[df$label,'x']) ** 2 + (df$y - model$centers[df$label,'y']) ** 2)
distances[i] <- mean(df$distance)
}

k.values <- 1:12
dist.df <- data.frame(k.values, distances)
