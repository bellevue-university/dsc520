...
## sex vs. earn
ggplot(heights_df, aes(x=sex, y=earn)) + geom_point()+ geom_boxplot(fill = "green", colour = "#3366FF")
## race vs. earn
ggplot(heights_df, aes(x=race, y=earn)) + geom_point() + geom_boxplot(aes(colour = race))

# https://ggplot2.tidyverse.org/reference/geom_bar.html
## Using `geom_bar()` plot a bar chart of the number of records for each `sex`
ggplot(heights_df, aes(sex)) + geom_bar(aes(fill = sex))

## Using `geom_bar()` plot a bar chart of the number of records for each race
ggplot(heights_df, aes(race)) + geom_bar() + geom_boxplot(aes(colour = race))

ggplot(heights_df, aes(race)) + geom_bar(aes(fill = race), position = position_stack(reverse = TRUE)) +
...
covid_df$date <- as.Date("2020-12-01")
...
ggplot(data=covid_df, aes(x=cases, group=1)) +
  ...
  scale_colour_manual("",
                      values = c("red", "blue", "green")) +
  xlab(" Cases") + ylab("Cases") + scale_y_log10()
